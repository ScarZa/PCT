function MatrixRegis2(content, id = null) {
    ...
    $("#submenu1").parent().find("li:eq(0)").addClass("active")
        ...
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-4 row'><label><input class='ace' type='date' name='regdate' placeholder='วันที่ลงทะเบียน'></label></div>"
            +"<div class='col-xs-6 row'><label><select name='m_type' class='form-control select2' id='m_type' required></select></label></div></div>")
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-6 row'><label><select name='m_status' class='form-control select2' id='m_status' required></select></label></div>"
        ...
        var option="<option value=''> เลือกบำบัด </option>";
                               option += "<option value='1'> สุรา </option><option value='2'> ยาเสพติด </option>";
                                       $("select#m_type").empty().append(option);
                                       $(".select2").select2();

        var option2="<option value=''> เลือกปีงบประมาณ </option>";
                                    var BY = new Date();
                                    var nowyear = BY.getFullYear()+544;
                                for(var i=0;i<=10;i++){
                               option2 += "<option value='"+nowyear+"'> "+nowyear+" </option>";
                               nowyear--;
                                }
...
$("#cgi-post").append($("<input type='hidden' name='hn' value='"+data[0].hn+"'>")
    ...
    ,$("<input type='hidden' name='method' value='regis_MR'>"));    
