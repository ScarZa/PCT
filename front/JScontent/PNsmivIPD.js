function PNSmivIPD(content, id = null) {

    var title = " งานผู้ป่วยใน (IPD)";
    var subtitle = "ตารางผู้ป่วยในที่เป็น SMI-V (IPD) ";
    ...
    AddPanel("DT_SMIV_search.php",$.cookie("ward"));
    $("select#sel-ward").change(function () {
        HAlert("#alert-add",$("#sel-ward").val());
        AddPanel("DT_SMIV.php",$("#sel-ward").val());
    });
    $("input#search-patient").keyup(function () {
        //if ($("#search-patient").val() != '') {
            AddPanel("DT_SMIV_search.php",$("#search-patient").val());
        //}
    });
