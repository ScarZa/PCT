function AddPanel(content,id = null) {
    $.getJSON('../back/API/'+content,{data1:id}, function (data) { 
        ...
        $.each(data, function (i, item) {
            ...
            , $("<div class='alert alert-danger'>เฝ้าระวัง : <span style='background-color:yellow'>" + item.typeP_1R + " " + item.typeP_2R + " " + item.typeP_3R + " " + item.typeP_4R + " " + item.typeP_5R + "</span><br><div><div id='smiv_class"+i+"'></div><span style='background-color:yellow' id='smiv-detial"+i+"'></span></div></div>")
        )
    ...
    if (data[0].smi5_4 != '') { $("#smiv-detial"+i).append("5.4 "+data[0].smi5_4 + "<br>") }
            ...
            if (item.ipd_fr_id){
                if (((item.typeP_1) + (item.typeP_2) + (item.typeP_3) + (item.typeP_4) + (item.typeP_5) > 0) || item.smi4_id) { $("#widget_box_" + i).addClass("widget-color-red3") } else { $("#widget_box_" + i).addClass("widget-color-green") }
            } else {
