function PatentRegis(content, id = null) {
    
    var title = " ลงทะเบียนงานสิทธิบัตร";
    var subtitle = "แบบลงทะเบียนงานสิทธิบัตร ";
    ...
    //$("#submenu1").parent().find("li:eq(0)").addClass("active")
        ...
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-4 row'><label><input class='ace' type='date' name='regdate' placeholder='วันที่ลงทะเบียน'></label></div>"
            +"<div class='col-xs-6 row'><label><select name='patent_type' class='form-control select2' id='patent_type' required></select></label></div></div>")
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-6 row'><label><select name='patent_status' class='form-control select2' id='patent_status' required></select></label></div>"
        ...
        selectMash("#patent_type","consult_data.php?data=016","เลือกเหตุผล");                      
        selectMash("#patent_status","member_status_data.php","เลือกสถานะ");
        selectMash("#doctor","clinic_data.php?data=016","เลือกผู้รับผิดชอบ");
...
$("#cgi-post").append($("<input type='hidden' name='hn' value='"+data[0].hn+"'>")
    ...
    ,$("<input type='hidden' name='method' value='regis_Patent'>"));    
