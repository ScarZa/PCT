function PNAccidentIPD(content, id = null) {

    var title = " งานผู้ป่วยใน (IPD)";
    var subtitle = "ตารางผู้ป่วยใน เฝ้าระวังอุบัติเหตุ (IPD) ";
    ...
    AddPanel("DT_Accident.php",$.cookie("ward"));
    $("select#sel-ward").change(function () {
        HAlert("#alert-add",$("#sel-ward").val());
        AddPanel("DT_Accident.php",$("#sel-ward").val());
    });
    $("input#search-patient").keyup(function () {
        //if ($("#search-patient").val() != '') {
            AddPanel("DT_Accident_search.php",$("#search-patient").val());
        //}
    });
