function SocialRegis2(content, id = null) {
    ...
    $("#submenu1").parent().find("li:eq(0)").addClass("active")
        ...
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-4 row'><label><input class='ace' type='date' name='regdate' placeholder='วันที่ลงทะเบียน'></label></div>"
            +"<div class='col-xs-6 row'><label><select name='soc_type' class='form-control select2' id='soc_type' required></select></label></div></div>")
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-6 row'><label><select name='soc_status' class='form-control select2' id='soc_status' required></select></label></div>"
        ...
        selectMash("#soc_type","consult_data.php?data=008","เลือกเหตุผล");                      
        selectMash("#soc_status","member_status_data.php","เลือกสถานะ");
        selectMash("#doctor","clinic_data.php?data=008","เลือกผู้รับผิดชอบ");
