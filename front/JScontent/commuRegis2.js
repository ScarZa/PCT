function CommuRegis2(content, id = null) {
    ...
    $("#submenu1").parent().find("li:eq(0)").addClass("active")
        ...
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-4 row'><label><input class='ace' type='date' name='regdate' placeholder='วันที่ลงทะเบียน'></label></div>"
            +"<div class='col-xs-6 row'><label><select name='comm_type' class='form-control select2' id='comm_type' required></select></label></div></div>")
        ,$("<div class='form-group col-xs-12 row'><div class='col-xs-6 row'><label><select name='comm_status' class='form-control select2' id='comm_status' required></select></label></div>"
...
$("#cgi-post").append($("<input type='hidden' name='hn' value='"+data[0].hn+"'>")
    ...
    ,$("<input type='hidden' name='method' value='regis_COMM'>"));    
