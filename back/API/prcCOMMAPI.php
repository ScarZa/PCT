...
if ($method == 'confirm_COMMU') {
    ...
    //$clinic=$conv->utf8_to_tis620('005');
    $lastupdate=  date("Y-m-d H:m:s");
    $status=$conv->utf8_to_tis620('Y');

    $sql = "SELECT commu_id FROM jvlcommunity_regis where hn ='".$hn ."' and regdate ='".$regdate."'";
        $connDB->imp_sql($sql);
...
//print_r($M_regis);
if($M_regis){
            $res = array("messege"=>'ลงทะเบียนในคลินิคจิตเวชชุมชนสำเร็จครับ!!!!',"check"=>'Y');
        }else{
            $res = array("messege"=>'บันทึกข้อมูลไม่สำเร็จครับ!!!!',"check"=>'N');
        }
}else{
