...
if ($method == 'confirm_Patent') {
        ...
        $connDB->close_PDO();
}elseif ($method == 'regis_Patent') {
    $hn=$conv->utf8_to_tis620($_POST['hn']);
    $tB_id=$conv->utf8_to_tis620($_POST['tB_id']);
    $patent_type=$conv->utf8_to_tis620($_POST['patent_type']);
    $regdate=$conv->utf8_to_tis620($_POST['regdate']);
    $patent_status=$conv->utf8_to_tis620($_POST['patent_status']);
    $doctor2 = $conv->utf8_to_tis620($_POST['doctor']);  
    $clinic=$conv->utf8_to_tis620('016');
    $lastupdate=  date("Y-m-d H:m:s");
    $status=$conv->utf8_to_tis620('Y');

    $sql = "SELECT patent_id FROM jvlpatent_regis where hn ='".$hn ."' and regdate ='".$regdate."'";
        $connDB->imp_sql($sql);
        $chk_send=$connDB->select_a();
        if(empty($chk_send)){
    $data = array($doctor2,$hn,$patent_type,$regdate,$patent_status,$tB_id);
    $table = "jvlpatent_regis";
...
// }
if($Phy_regis){
            $res = array("messege"=>'ลงทะเบียนในงานสิทธิบัตรสำเร็จครับ!!!!',"check"=>'Y');
        }else{
            $res = array("messege"=>'บันทึกข้อมูลไม่สำเร็จครับ!!!!',"check"=>'N');
        }
}else{
