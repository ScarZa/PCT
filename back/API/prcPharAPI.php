...
if ($method == 'confirm_Phar') {
    ...
    $sql = "SELECT phar_id FROM jvlphar_regis where hn ='".$hn ."' and regdate ='".$regdate."'";
        $connDB->imp_sql($sql);
...
// }
if($Phar_regis){
            $res = array("messege"=>'ลงทะเบียนในงานเภสัชกรรมสำเร็จครับ!!!!',"check"=>'Y');
        }else{
            $res = array("messege"=>'บันทึกข้อมูลไม่สำเร็จครับ!!!!',"check"=>'N');
        }
}else{
