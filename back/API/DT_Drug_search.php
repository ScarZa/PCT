...
if(!empty($data)){
  $data =$conv->utf8_to_tis620($data);
    $code = "and (a.an like '%".$data."%' or p.fname like '%".$data."%' or p.lname like '%".$data."%' or a.hn like '%".$data."%')";
}else{
    $code ='';
}

$sql="select fr.ipd_fr_id,a.an,a.vn,a.hn,a.regdate
,(SELECT count(a2.hn)
FROM an_stat a2
...
from an_stat a 
inner join opitemrece op on a.an = op.an
