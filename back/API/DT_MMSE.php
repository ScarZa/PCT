...
,(SELECT count(mmse.hn) count FROM jvl_mmse mmse WHERE mmse.hn = p.hn)count
from patient p
inner join jvl_mmse mmse on mmse.hn = p.hn
inner JOIN vn_stat v ON v.hn=p.hn
inner join pttype pt on v.pttype=pt.pttype
GROUP BY mmse.hn ORDER BY mmse.mmse_id desc"; 
$conn_DB->imp_sql($sql);
$num_risk = $conn_DB->select();

    // $sql2="SELECT count(hn) count FROM jvl_mmse GROUP BY hn ORDER BY mmse_id desc"; 
